...
затем sweep-line объединяет пересекающиеся интервалы в цепочки. Все события в
одной цепочке получают общий `simultaneousGroupId`.
...
конфликт, а не одновременная лекция.
